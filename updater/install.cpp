...
static bool make_parents(const std::string & name)
{
  ...
  while (prev_end < name.size())
      ...
      if (!is_dir(dir_path))
	{
	  ...
	  if (result != 0)
	    {
	      PLOG(ERROR) << "failed to mkdir " << dir_path <<
		" when make parents for " << name;
	      return false;
	    }
...
Value *MountFn(const char *name, State * state,
	       const std::vector < std::unique_ptr < Expr >> &argv)
{
  if (argv.size() != 4 && argv.size() != 5)
    {
      return ErrorAbort(state, kArgsParsingFailure,
			"%s() expects 4-5 args, got %zu", name, argv.size());
    }
  ...
  if (!ReadArgs(state, argv, &args))
    {
      return ErrorAbort(state, kArgsParsingFailure,
			"%s() Failed to parse the argument(s)", name);
    }
  ...
  if (fs_type.empty())
    {
      return ErrorAbort(state, kArgsParsingFailure,
			"fs_type argument to %s() can't be empty", name);
    }
  ...
  if (location.empty())
    {
      return ErrorAbort(state, kArgsParsingFailure,
			"location argument to %s() can't be empty", name);
    }
  ...
  if (mount
      (location.c_str(), mount_point.c_str(), fs_type.c_str(),
       MS_NOATIME | MS_NODEV | MS_NODIRATIME, mount_options.c_str()) < 0)
    {
      uiPrintf(state, "%s: Failed to mount %s at %s: %s", name,
	       location.c_str(), mount_point.c_str(), strerror(errno));
      return StringValue("");
    }
...
Value *IsMountedFn(const char *name, State * state,
		   const std::vector < std::unique_ptr < Expr >> &argv)
{
  if (argv.size() != 1)
    {
      return ErrorAbort(state, kArgsParsingFailure,
			"%s() expects 1 arg, got %zu", name, argv.size());
    }
  ...
  MountedVolume *vol =
    find_mounted_volume_by_mount_point(mount_point.c_str());
  if (vol == nullptr)
    {
      return StringValue("");
    }
...
Value *UnmountFn(const char *name, State * state,
		 const std::vector < std::unique_ptr < Expr >> &argv)
{
  if (argv.size() != 1)
    {
  ...
  if (vol == nullptr)
    {
      uiPrintf(state, "Failed to unmount %s: No such volume",
	       mount_point.c_str());
      return nullptr;
    }
  else
    {
      ...
      if (ret != 0)
	{
	  uiPrintf(state, "Failed to unmount %s: %s", mount_point.c_str(),
		   strerror(errno));
	}
    }
...
Value *FormatFn(const char *name, State * state,
		const std::vector < std::unique_ptr < Expr >> &argv)
{
  if (argv.size() != 5)
    {
      return ErrorAbort(state, kArgsParsingFailure,
			"%s() expects 5 args, got %zu", name, argv.size());
    }
  ...
  if (fs_type == "ext4")
    {
      const char *mke2fs_argv[] =
	{ "/sbin/mke2fs_static", "-t", "ext4", "-b", "4096",
      ...
      if (size != 0)
	{
      ...
      int status =
	exec_cmd(mke2fs_argv[0], const_cast < char **>(mke2fs_argv));
      if (status != 0)
	{
	  LOG(WARNING) << name << ": mke2fs failed (" << status << ") on " <<
	    location << ", falling back to make_ext4fs";
	  status =
	    make_ext4fs(location.c_str(), size, mount_point.c_str(),
			sehandle);
	  if (status != 0)
	    {
	      LOG(ERROR) << name << ": make_ext4fs failed (" << status <<
		") on " << location;
	      return StringValue("");
	    }
      ...
      const char *e2fsdroid_argv[] =
	{ "/sbin/e2fsdroid_static", "-e", "-a", mount_point.c_str(),
	location.c_str(), nullptr
      };
      status =
	exec_cmd(e2fsdroid_argv[0], const_cast < char **>(e2fsdroid_argv));
      if (status != 0)
	{
	  LOG(ERROR) << name << ": e2fsdroid failed (" << status << ") on " <<
	    location;
	  return StringValue("");
	}
      return StringValue(location);
  ...
  else if (fs_type == "f2fs")
    {
      if (size < 0)
	{
	  LOG(ERROR) << name << ": fs_size can't be negative for f2fs: " <<
	    fs_size;
	  return StringValue("");
	}
	...
	nullptr
      ...
      if (status != 0)
	{
	  LOG(ERROR) << name << ": mkfs.f2fs failed (" << status << ") on " <<
	    location;
	  return StringValue("");
	}
      return StringValue(location);
    ...
    {
      LOG(ERROR) << name << ": unsupported fs_type \"" << fs_type <<
		      ...
		      const std::vector < std::unique_ptr < Expr >> &argv)
{
  if (argv.size() != 2)
    {
      return ErrorAbort(state, kArgsParsingFailure,
			"%s() expects 2 args, got %zu", name, argv.size());
    }
			    ...
			    Expr >> &argv)
{
  if (argv.size() < 1 || argv.size() > 2)
    {
      return ErrorAbort(state, kArgsParsingFailure,
			"%s() expects 1 or 2 args, got %zu", name,
			argv.size());
    }
  ...
  if (argv.size() == 2)
	...
	{
	  return ErrorAbort(state, kArgsParsingFailure,
			    "%s() Failed to parse %zu args", name,
			    argv.size());
	}
      ...
      ZipArchiveHandle za =
	static_cast < UpdaterInfo * >(state->cookie)->package_zip;
	...
	fd(TEMP_FAILURE_RETRY
	   (ota_open
	    (dest_path.c_str(), O_WRONLY | O_CREAT | O_TRUNC,
	     S_IRUSR | S_IWUSR)));
      if (fd == -1)
	{
      ...
      if (ret != 0)
	{
	  LOG(ERROR) << name << ": Failed to extract entry \"" << zip_path <<
	    "\" (" << entry.
	    uncompressed_length << " bytes) to \"" << dest_path << "\": " <<
	    ErrorCodeString(ret);
	  success = false;
	}
      if (ota_fsync(fd) == -1)
      ...
      if (FindEntry(za, zip_string_path, &entry) != 0)
	{
	  return ErrorAbort(state, kPackageExtractFileFailure,
			    "%s(): no %s in package", name, zip_path.c_str());
	}
      ...
      int32_t ret =
	ExtractToMemory(za, &entry,
			reinterpret_cast < uint8_t * >(&buffer[0]),
			buffer.size());
      if (ret != 0)
	{
	  return ErrorAbort(state, kPackageExtractFileFailure,
			    "%s: Failed to extract entry \"%s\" (%zu bytes) to memory: %s",
			    name, zip_path.c_str(), buffer.size(),
			    ErrorCodeString(ret));
	}
...
Value *GetPropFn(const char *name, State * state,
		 const std::vector < std::unique_ptr < Expr >> &argv)
{
  if (argv.size() != 1)
    {
		     ...
		     const std::vector < std::unique_ptr < Expr >> &argv)
{
  ...
  if (st.st_size > MAX_FILE_GETPROP_SIZE)
    {
      return ErrorAbort(state, kFileGetPropFailure,
			"%s too large for %s (max %lld)", filename.c_str(),
			name,
			static_cast < long long >(MAX_FILE_GETPROP_SIZE));
    }
  ...
  if (f == nullptr)
    {
      return ErrorAbort(state, kFileOpenFailure, "%s: failed to open %s: %s",
			name, filename.c_str(), strerror(errno));
    }

  if (ota_fread(&buffer[0], 1, st.st_size, f.get()) != static_cast < size_t >
      (st.st_size))
    {
			 ...
			 const std::vector < std::unique_ptr < Expr >> &argv)
{
  if (argv.size() != 1)
    {
      return ErrorAbort(state, kArgsParsingFailure,
			"%s() expects 1 args, got %zu", name, argv.size());
    }
  std::vector < std::string > args;
  if (!ReadArgs(state, argv, &args))
    {
		    ...
		    const std::vector < std::unique_ptr < Expr >> &argv)
{
  ...
  if (!android::base::ParseUint(target_size_str.c_str(), &target_size))
    {
      return ErrorAbort(state, kArgsParsingFailure,
			"%s(): can't parse \"%s\" as byte count", name,
  ...
  for (int i = 0; i < patchcount; ++i)
	...
	{
	  return ErrorAbort(state, kArgsParsingFailure,
			    "%s(): sha-1 #%d is not string", name, i * 2);
	}
      if (arg_values[i * 2 + 1]->type != VAL_BLOB)
	{
	  return ErrorAbort(state, kArgsParsingFailure,
			    "%s(): patch #%d is not blob", name, i * 2 + 1);
	}
    }
			 ...
			 const std::vector < std::unique_ptr < Expr >> &argv)
{
  if (argv.size() < 1)
    {
      return ErrorAbort(state, kArgsParsingFailure,
			"%s(): expected at least 1 arg, got %zu", name,
			argv.size());
    }
...
Value *UIPrintFn(const char *name, State * state,
		 const std::vector < std::unique_ptr < Expr >> &argv)
{
  std::vector < std::string > args;
  if (!ReadArgs(state, argv, &args))
...
Value *WipeCacheFn(const char *name, State * state,
		   const std::vector < std::unique_ptr < Expr >> &argv)
{
  if (!argv.empty())
    {
      return ErrorAbort(state, kArgsParsingFailure,
			"%s() expects no args, got %zu", name, argv.size());
    }
  fprintf(static_cast < UpdaterInfo * >(state->cookie)->cmd_pipe,
	  "wipe_cache\n");
		    ...
		    const std::vector < std::unique_ptr < Expr >> &argv)
{
  ...
  LOG(INFO) << "about to run program [" << args2[0] << "] with " << argv.
    size() << " args";
  ...
  if (WIFEXITED(status))
	...
	{
	  LOG(ERROR) << "run_program: child exited with status " <<
	    WEXITSTATUS(status);
	}
    }
  else if (WIFSIGNALED(status))
    {
      LOG(ERROR) << "run_program: child terminated by signal " <<
	WTERMSIG(status);
    }
...
//
Value *Sha1CheckFn(const char *name, State * state,
		   const std::vector < std::unique_ptr < Expr >> &argv)
{
  if (argv.size() < 1)
    {
  ...
  SHA1(reinterpret_cast < const uint8_t * >(args[0]->data.c_str()),
       args[0]->data.size(), digest);
...
Value *ReadFileFn(const char *name, State * state,
		  const std::vector < std::unique_ptr < Expr >> &argv)
{
  if (argv.size() != 1)
    {
		    ...
		    const std::vector < std::unique_ptr < Expr >> &argv)
{
  ...
  if (filename.empty())
    {
      return ErrorAbort(state, kArgsParsingFailure,
			"%s(): Filename cannot be empty", name);
    }
...
// partition.
Value *RebootNowFn(const char *name, State * state,
		   const std::vector < std::unique_ptr < Expr >> &argv)
{
  if (argv.size() != 2)
    {
  ...
  if (!read_bootloader_message_from(&boot, filename, &err))
    {
      LOG(ERROR) << name << "(): Failed to read from \"" << filename << "\": "
	<< err;
      return StringValue("");
    }
  ...
  if (!write_bootloader_message_to(boot, filename, &err))
    {
      LOG(ERROR) << name << "(): Failed to write to \"" << filename << "\": "
	<< err;
      return StringValue("");
    }
...
// bytes.
Value *SetStageFn(const char *name, State * state,
		  const std::vector < std::unique_ptr < Expr >> &argv)
{
  if (argv.size() != 2)
    {
...
Value *GetStageFn(const char *name, State * state,
		  const std::vector < std::unique_ptr < Expr >> &argv)
{
  if (argv.size() != 1)
    {
...
Value *Tune2FsFn(const char *name, State * state,
		 const std::vector < std::unique_ptr < Expr >> &argv)
{
#ifdef HAVE_LIBTUNE2FS
  if (argv.empty())
    {
      return ErrorAbort(state, kArgsParsingFailure,
			"%s() expects args, got %zu", name, argv.size());
    }
  ...
  return StringValue("t");
#else
  return ErrorAbort(state, kTune2FsFailure,
		    "%s() support not present, no libtune2fs", name);
