...
int DataManager::ShowProgress(const float Portion, const float Seconds)
{
  ...
  if (SetValue
      ("ui_progress_portion",
       (float) (Portion * 100.0) + Starting_Portion) != 0)
    return -1;
...
void DataManager::SetDefaultValues()
{
  ...
  mPersist.SetValue(FOX_DISABLE_OTA_AUTO_REBOOT, "0"); // DJ9 disable auto-reboot after OTA updates? set to 1 to enable this feature
  // { MIUI
...
void DataManager::ReadSettingsFile(void)
{
  ...
  sprintf(settings_file, "%s/.foxs", mkdir_path);
