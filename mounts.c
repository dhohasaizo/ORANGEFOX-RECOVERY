...
static inline void free_volume_internals(const MountedVolume * volume,
					 int zero)
{
  ...
  while (nbytes > 0)
    {
       ...
       */
      matches =
	sscanf(bufp, "%63s %63s %63s %127s", device, mount_point, filesystem,
	       flags);
...
int remount_read_only(const MountedVolume * volume)
{
  return mount(volume->device, volume->mount_point, volume->filesystem,
	       MS_NOATIME | MS_NODEV | MS_NODIRATIME | MS_RDONLY | MS_REMOUNT,
	       0);
}
