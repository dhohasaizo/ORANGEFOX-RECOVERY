...
bool read_metadata_from_package(ZipArchiveHandle zip, std::string * metadata)
{
  ...
  static constexpr const char *METADATA_PATH =
  ...
  int32_t err =
    ExtractToMemory(zip, &entry,
		    reinterpret_cast < uint8_t * >(&(*metadata)[0]), length);
  if (err != 0)
    {
      LOG(ERROR) << "Failed to extract " << METADATA_PATH << ": " <<
	ErrorCodeString(err);
      return false;
    }
  return true;
...
static void read_source_target_build(ZipArchiveHandle zip,
...
for (const std::string & line:android::base::Split(metadata_str, "\n"))
  ...
  if (!pkg_pre_build.empty() && pkg_pre_build != value)
    {
      LOG(ERROR) << "Package is for source build " << pkg_pre_build <<
	" but expected " << value;
      return INSTALL_ERROR;
    }
  ...
  if (!pkg_pre_build_fingerprint.empty()
	...
	pkg_pre_build_fingerprint << " but expected " << value;
      return INSTALL_ERROR;
    }
			  ...
			  std::vector < std::string > *cmd)
{
  ...
  // in the zip file.
  static constexpr const char *AB_OTA_PAYLOAD_PROPERTIES =
  ...
  int32_t err =
    ExtractToMemory(zip, &properties_entry, payload_properties.data(),
  ...
  *cmd =
  {
  binary_path, "--payload=file://" + package,
...
int update_binary_command(const std::string & package, ZipArchiveHandle zip,
			  const std::string & binary_path, int retry_count,
			  int status_fd, std::vector < std::string > *cmd)
{
  CHECK(cmd != nullptr);

  // On traditional updates we extract the update binary from the package.
  static constexpr const char *UPDATE_BINARY_NAME =
  ...
  if (error != 0)
    {
      LOG(ERROR) << "Failed to extract " << UPDATE_BINARY_NAME << ": " <<
...
static void log_max_temperature(int *max_temperature,
				const std::atomic < bool > &logger_finished)
{
  CHECK(max_temperature != nullptr);
  std::mutex mtx;
  std::unique_lock < std::mutex > lck(mtx);
  while (!logger_finished.load()
	 && finish_log_temperature.wait_for(lck,
					    20 s) == std::cv_status::timeout)
    {
      *max_temperature =
	std::max(*max_temperature, GetMaxValueFromThermalZone());
    }
}

// If the package contains an update binary, extract it and run it.
static int try_update_binary(const std::string & package,
			     ZipArchiveHandle zip, bool * wipe_cache,
			     std::vector < std::string > *log_buffer,
...
#ifdef AB_OTA_UPDATER
  int ret =
    update_binary_command(package, zip, "/sbin/update_engine_sideload",
			  retry_count,
			  pipefd[1], &args);
#else
  int ret =
    update_binary_command(package, zip, "/tmp/update-binary", retry_count,
			  pipefd[1],
			  &args);
#endif
  if (ret) {
  ...
  *wipe_cache = false;
  ...
  while (fgets(buffer, sizeof(buffer), from_child) != nullptr)
	...
	continue;
      ...
      std::string args =
	space ==
	std::string::npos ? "" : android::base::Trim(line.substr(space));

      if (command == "progress")
	{
	  std::vector < std::string > tokens =
	    android::base::Split(args, " ");
	  double fraction;
	  int seconds;
	  if (tokens.size() == 2
	      && android::base::ParseDouble(tokens[0].c_str(), &fraction)
	      && android::base::ParseInt(tokens[1], &seconds))
	    {
	      ui->ShowProgress(fraction *
			       (1 - VERIFICATION_PROGRESS_FRACTION), seconds);
	    }
	  else
	    {
  ...
  if (!WIFEXITED(status) || WEXITSTATUS(status) != 0)
    {
      LOG(ERROR) << "Error in " << package << " (Status " <<
	WEXITSTATUS(status) << ")";
      return INSTALL_ERROR;
    }
...
bool verify_package_compatibility(ZipArchiveHandle package_zip)
{
  ...
  if (FindEntry(package_zip, compatibility_entry_name, &compatibility_entry)
      != 0)
    {
      LOG(INFO) << "Package doesn't contain " << COMPATIBILITY_ZIP_ENTRY <<
	" entry";
      return true;
    }
  ...
  int32_t ret;
  if ((ret =
       ExtractToMemory(package_zip, &compatibility_entry,
		       reinterpret_cast < uint8_t * >(&zip_content[0]),
		       compatibility_entry.uncompressed_length)) != 0)
    {
      LOG(ERROR) << "Failed to read " << COMPATIBILITY_ZIP_ENTRY << ": " <<
	ErrorCodeString(ret);
      return false;
    }
  ...
  ret =
    OpenArchiveFromMemory(static_cast <
			  void *>(const_cast < char *>(zip_content.data())),
			  zip_content.size(), COMPATIBILITY_ZIP_ENTRY,
			  &zip_handle);
  if (ret != 0)
    {
      LOG(ERROR) << "Failed to OpenArchiveFromMemory: " <<
	ErrorCodeString(ret);
      return false;
    }
  ...
  std::unique_ptr < void, decltype(&EndIteration) > guard(cookie,
							  EndIteration);
  ...
  while (Next(cookie, &info_entry, &info_name) == 0)
    {
      ...
      int32_t ret =
	ExtractToMemory(zip_handle, &info_entry,
			reinterpret_cast < uint8_t * >(&content[0]),
			info_entry.uncompressed_length);
      if (ret != 0)
	{
	  LOG(ERROR) << "Failed to read " << info_name.
	    name << ": " << ErrorCodeString(ret);
	  CloseArchive(zip_handle);
	  return false;
	}
  ...
  std::string err;
  int result =
    android::vintf::VintfObjectRecovery::
    CheckCompatibility(compatibility_info, &err);
  if (result == 0)
    {
      return true;
    }

  LOG(ERROR) << "Failed to verify package compatibility (result " << result <<
    "): " << err;
  return false;
}

static int really_install_package(const std::string & path, bool * wipe_cache,
				  bool needs_mount,
				  std::vector < std::string > *log_buffer,
				  int retry_count, int *max_temperature)
{
  ...
  ui->ShowProgress(VERIFICATION_PROGRESS_FRACTION,
  ...
  if (!verify_package(map.addr, map.length))
	...
	push_back(android::base::
		  StringPrintf("error: %d", kZipVerificationFailure));
      return INSTALL_CORRUPT;
    }
  ...
  if (err != 0)
    {
      ...
      log_buffer->
	push_back(android::base::StringPrintf("error: %d", kZipOpenFailure));
  ...
  int result =
    try_update_binary(path, zip, wipe_cache, log_buffer, retry_count,
		      max_temperature);
  ui->SetEnableReboot(true);
  ui->Print("\n");
...
int install_package(const std::string & path, bool * wipe_cache,
		    const std::string & install_file, bool needs_mount,
		    int retry_count)
{
  CHECK(!path.empty());
  ...
  if (setup_install_mounts() != 0)
    ...
    {
      result =
	really_install_package(path, wipe_cache, needs_mount, &log_buffer,
  ...
  if (has_cache)
    {
      static constexpr const char *UNCRYPT_STATUS =
	"/cache/recovery/uncrypt_status";
      ...
      else
	{
	  ...
	  if (!android::base::
	      ReadFileToString(UNCRYPT_STATUS, &uncrypt_status))
	    {
  ...
  {
  path, result == INSTALL_SUCCESS ? "1" : "0",
  ...
  if (start_temperature > 0)
    {
      log_buffer.push_back("temperature_start: " +
			   std::to_string(start_temperature));
    }
  if (end_temperature > 0)
    {
      log_buffer.push_back("temperature_end: " +
			   std::to_string(end_temperature));
    }
  if (max_temperature > 0)
    {
      log_buffer.push_back("temperature_max: " +
...
bool verify_package(const unsigned char *package_data, size_t package_size)
{
  ...
  LOG(INFO) << loadedKeys.
    size() << " key(s) loaded from " << PUBLIC_KEYS_FILE;
