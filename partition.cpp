...
bool TWPartition::Process_Fstab_Line(const char *fstab_line,
				     bool Display_Error, std::map < string,
				     Flags_Map > *twrp_flags)
{
  ...
  bool skip = false;
  int fstab_version = 1, mount_point_index = 0, fs_index =
    1, block_device_index = 2;
      ...
      else if (item_index == block_device_index)
	{
	  ...
	  if (Fstab_File_System == "mtd" || Fstab_File_System == "yaffs2")
		...
		MTD_Name = "recovery";
	      ...
	      if (*ptr != '/')
		LOGERR
		  ("Until we get better BML support, you will have to find and provide the full block device path to the BML devices e.g. /dev/block/bml9 instead of the partition name\n");
	    }
	  else if (*ptr != '/')
	    {
	      if (Display_Error)
		LOGERR("Invalid block device '%s' in fstab line '%s'", ptr,
		       fstab_line);
	      else
		LOGINFO("Invalid block device '%s' in fstab line '%s'", ptr,
			fstab_line);
	      return false;
	    }
	  else
      ...
      else if (item_index > 2)
	{
	  if (fstab_version == 2)
	      ...
	      if (item_index == 3)
		{
		  ...
		  if (additional_entry)
		    {
		      additional_entry->Save_FS_Flags(Fstab_File_System,
						      Mount_Flags,
						      Mount_Options);
  ...
  if (Mount_Point == "auto")
      ...
      if (Mount_Point == EXPAND(TW_EXTERNAL_STORAGE_PATH))
	{
	  ...
	  Wipe_Available_in_GUI = true;
#else
      if (Mount_Point == "/sdcard" || Mount_Point == "/external_sd"
	  || Mount_Point == "/external_sdcard")
	{
	  Is_Storage = true;
	  Removable = true;
	  Wipe_Available_in_GUI = true;
...
#else
      if (Mount_Point == "/emmc" || Mount_Point == "/internal_sd"
	  || Mount_Point == "/internal_sdcard")
	{
	  Is_Storage = true;
	  Is_Settings_Storage = true;
	  Wipe_Available_in_GUI = true;
  ...
  if (strlen(twflags) > 0)
	...
	Display_Name = Prev_Display_Name;
      if (!has_storage_name)
	Storage_Name = Prev_Storage_Name;
      if (!has_backup_name)
	Backup_Display_Name = Prev_Backup_Display_Name;
  ...
  if (fstab_version == 2 && twrp_flags && twrp_flags->size() > 0)
      ...
      if (it != twrp_flags->end())
	{
	  ...
	  if (strlen(twrpflags) > strlen("flags=")
	      && strncmp(twrpflags, "flags=", strlen("flags=")) == 0)
	    skip += strlen("flags=");
...
void TWPartition::Process_FS_Flags(const char *str)
{
  ...
  for (ptr = strtok_r(options, ",", &savep); ptr;
	...
	name_len = equals - ptr;
	   ...
	   *ignored_mount_item; ignored_mount_item++)
	{
...
void TWPartition::Save_FS_Flags(const string & local_File_System,
				int local_Mount_Flags,
				const string & local_Mount_Options)
{
	  ...
	  if (strncmp(ptr, tw_flag->name, flag_len) == 0)
	      ...
	      else if (ptr_len > flag_len && ptr[flag_len] == '=')
		{
		  ...
		  if (strlen(ptr) == 0)
		    {
		      LOGINFO
			("Flag missing argument or should not include '=': %s=\n",
			 tw_flag->name);
		      break;
		    }
  ...
  while (fgets(line, sizeof(line), fp) != NULL)
      ...
      else
	{
	  ...
	  while (tmpString[space_count] == 32)
	    space_count++;
	  sscanf(line + space_count, "%lu %lu %lu", &blocks, &used,
		 &available);
	}
  ...
  if (fp != NULL)
      ...
      while (fgets(line, sizeof(line), fp) != NULL)
	{
	  ...
	  if (tmpdevice == Primary_Block_Device
	      || tmpdevice == Alternate_Block_Device)
	    {
	      Size = size;
	      fclose(fp);
	      return true;
...
bool TWPartition::Mount(bool Display_Error)
{
  ...
  if (Current_File_System == "exfat"
      && TWFunc::Path_Exists("/sbin/exfat-fuse"))
    {
      string cmd =
	"/sbin/exfat-fuse -o big_writes,max_read=131072,max_write=131072 " +
	Actual_Block_Device + " " + Mount_Point;
      LOGINFO("cmd: %s\n", cmd.c_str());
      string result;
      if (TWFunc::Exec_Cmd(cmd, result) != 0)
	{
	  LOGINFO
	    ("exfat-fuse failed to mount with result '%s', trying vfat\n",
	     result.c_str());
  ...
  if (Current_File_System == "ntfs"
      && (TWFunc::Path_Exists("/sbin/ntfs-3g")
	  || TWFunc::Path_Exists("/sbin/mount.ntfs")))
    {
      string cmd;
	...
	Ntfsmount_Binary = "mount.ntfs";
      ...
      if (Mount_Read_Only)
	cmd =
	  "/sbin/" + Ntfsmount_Binary + " -o ro " + Actual_Block_Device +
	  " " + Mount_Point;
      else
	cmd =
	  "/sbin/" + Ntfsmount_Binary + " " + Actual_Block_Device + " " +
	  Mount_Point;
      LOGINFO("cmd: '%s'\n", cmd.c_str());
  ...
  if (Fstab_File_System == "yaffs2")
	...
	flags |= MS_RDONLY;
	  ...
	  (Actual_Block_Device.c_str(), Mount_Point.c_str(),
	   Fstab_File_System.c_str(), flags, NULL) < 0)
	{
	  if (mount
	      (Actual_Block_Device.c_str(), Mount_Point.c_str(),
	       Fstab_File_System.c_str(), flags | MS_RDONLY, NULL) < 0)
	    {
	      if (Display_Error)
		gui_msg(Msg
			(msg::kError,
  ...
  if (Current_File_System == "exfat"
      && TWFunc::Path_Exists("/sys/module/texfat"))
    mount_fs = "texfat";

  if (!exfat_mounted
      && mount(Actual_Block_Device.c_str(), Mount_Point.c_str(),
	       mount_fs.c_str(), flags, Mount_Options.c_str()) != 0
      && mount(Actual_Block_Device.c_str(), Mount_Point.c_str(),
	       mount_fs.c_str(), flags, NULL) != 0)
    {
#ifdef TW_NO_EXFAT_FUSE
      if (Current_File_System == "exfat")
	{
	  ...
	  if (mount
	      (Actual_Block_Device.c_str(), Mount_Point.c_str(), "vfat", 0,
	       NULL) != 0)
	    {
	      if (Display_Error)
		gui_msg(Msg
      ...
      else
	{
	  ...
	  if (!Removable && Display_Error)
	    gui_msg(Msg(msg::kError, "fail_mount=Failed to mount '{1}' ({2})")
		    (Mount_Point) (strerror(errno)));
	  else
	    LOGINFO("Unable to mount '%s'\n", Mount_Point.c_str());
  ...
  if (Is_Mounted())
      ...
      if (Is_Mounted())
	{
	  if (Display_Error)
	    gui_msg(Msg
		    (msg::kError,
		     "fail_unmount=Failed to unmount '{1}' ({2})")
		    (Mount_Point) (strerror(errno)));
	  else
...
bool TWPartition::Wipe(string New_File_System)
{
  ...
  if (Retain_Layout_Version && Mount(false)
      && TWFunc::Path_Exists(Layout_Filename))
...
bool TWPartition::Can_Repair()
{
  ...
  if (Current_File_System == "vfat" && TWFunc::Path_Exists("/sbin/fsck.fat"))
	...
	&& TWFunc::Path_Exists("/sbin/e2fsck"))
    return true;
  else if (Current_File_System == "exfat"
	   && TWFunc::Path_Exists("/sbin/fsck.exfat"))
    return true;
  else if (Current_File_System == "f2fs"
	   && TWFunc::Path_Exists("/sbin/fsck.f2fs"))
    return true;
  else if (Current_File_System == "ntfs"
	   && (TWFunc::Path_Exists("/sbin/ntfsfix")
	       || TWFunc::Path_Exists("/sbin/fsck.ntfs")))
    return true;
  return false;
}
...
bool TWPartition::Repair()
{
  ...
  if (Current_File_System == "vfat")
      ...
      if (!TWFunc::Path_Exists("/sbin/fsck.fat"))
	{
	  gui_msg(Msg
		  (msg::kError,
		   "repair_not_exist={1} does not exist! Cannot repair!")
		  ("fsck.fat"));
	  return false;
	}
      if (!UnMount(true))
	return false;
      gui_msg(Msg("repairing_using=Repairing {1} using {2}...") (Display_Name)
	      ("fsck.fat"));
      Find_Actual_Block_Device();
	...
	{
	  gui_msg(Msg(msg::kError, "unable_repair=Unable to repair {1}.")
		  (Display_Name));
	  return false;
	}
    }
...
bool TWPartition::Can_Resize()
{
  ...
  if ((Current_File_System == "ext2" || Current_File_System == "ext3"
       || Current_File_System == "ext4")
      && TWFunc::Path_Exists("/sbin/resize2fs"))
    return true;
  return false;
}
...
bool TWPartition::Resize()
{
  ...
  if (Current_File_System == "ext2" || Current_File_System == "ext3"
	...
	{
	  LOGINFO
	    ("Cannot resize %s because %s cannot be repaired before resizing.\n",
	     Display_Name.c_str(), Display_Name.c_str());
	  gui_msg(Msg(msg::kError, "cannot_resize=Cannot resize {1}.")
		  (Display_Name));
	  return false;
	}
      if (!TWFunc::Path_Exists("/sbin/resize2fs"))
	{
      ...
      gui_msg(Msg("repair_resize=Repairing {1} before resizing.")
	      (Display_Name));
      if (!Repair())
	return false;
      gui_msg(Msg("resizing=Resizing {1} using {2}...") (Display_Name)
	      ("resize2fs"));
      Find_Actual_Block_Device();
	  ...
	  if (Length < 0)
	    {
	      ...
	      Block_Count =
		(Actual_Size / 1024LLU) -
		((unsigned long long) (Length * -1) / 1024LLU);
	    }
	  else
	    {
	...
	{
	  Update_Size(true);
	  gui_msg(Msg(msg::kError, "unable_resize=Unable to resize {1}.")
		  (Display_Name));
	  return false;
	}
    }
...
string TWPartition::Get_Restore_File_System(PartitionSettings * part_settings)
{
  ...
  Restore_File_System =
    Backup_FileName.substr(first_period + 1,
			   Backup_FileName.size() - first_period - 1);
...
bool TWPartition::
Wipe_Data_Without_Wiping_Media_Func(const string & parent __unused)
{
  string dir;
	  ...
	  if (de->d_type == DT_DIR)
	      ...
	      if (!Wipe_Data_Without_Wiping_Media_Func(dir))
		{
	      ...
	      rmdir(dir.c_str());
		   ...
		   || de->d_type == DT_FIFO || de->d_type == DT_SOCK)
	    {
  ...
  gui_msg(Msg(msg::kError, "error_opening_strerr=Error opening: '{1}' ({2})")
	  (Mount_Point) (strerror(errno)));
  return false;
}
...
bool TWPartition::Raw_Read_Write(PartitionSettings * part_settings)
{
  ...
  if (src_fd < 0)
    {
      gui_msg(Msg
	      (msg::kError,
	       "error_opening_strerr=Error opening: '{1}' ({2})") (srcfn.
								   c_str())
	      (strerror(errno)));
      return false;
    }
  ...
  dest_fd =
    open(destfn.c_str(), O_WRONLY | O_CREAT | O_TRUNC | O_LARGEFILE,
	 S_IRUSR | S_IWUSR);
  if (dest_fd < 0)
    {
      gui_msg(Msg
	      (msg::kError,
	       "error_opening_strerr=Error opening: '{1}' ({2})") (destfn.
								   c_str())
	      (strerror(errno)));
      goto exit;
    }
  ...
  if (part_settings->progress)
    part_settings->progress->SetPartitionSize(part_settings->
					      total_restore_size);
  ...
  if (TWFunc::Get_File_Size(Full_FileName) == 0)
    {
      ...
      gui_msg(Msg
	      (msg::kError,
	       "backup_size=Backup file size for '{1}' is 0 bytes.")
	      (Full_FileName));
      return false;
    }
  if (part_settings->progress)
...
unsigned long long TWPartition::Get_Restore_Size(PartitionSettings *
						 part_settings)
{
  if (!part_settings->adbbackup)
    {
      InfoManager restore_info(part_settings->Backup_Folder + "/" +
			       Backup_Name + ".info");
...
bool TWPartition::Restore_Tar(PartitionSettings * part_settings)
{
  ...
  if (Has_Android_Secure)
      ...
      if (DataManager::GetIntValue(FOX_RUN_SURVIVAL_BACKUP) != 1)
	{
	  ...
	  && Restore_File_System != Current_File_System)
	{
	  gui_msg(Msg
		  (msg::kWarning,
  ...
  if (Mount_Point == PartitionManager.Get_Android_Root_Path() && Mount(true)
      && TWFunc::Path_Exists("/system/bin/run-as"))
    {
      ...
      if (setxattr
	  ("/system/bin/run-as", XATTR_NAME_CAPS, &cap_data, sizeof(cap_data),
	   0) < 0)
	{
	  LOGINFO
	    ("Failed to reset capabilities of /system/bin/run-as binary.\n");
	}
      else
	{
	  LOGINFO
	    ("Reset capabilities of /system/bin/run-as binary successful.\n");
	}
    }
#endif
...
bool TWPartition::Restore_Image(PartitionSettings * part_settings)
{
  ...
  if (Restore_File_System == "emmc")
	...
	part_settings->total_restore_size =
	  (uint64_t) (TWFunc::Get_File_Size(Full_FileName));
      if (!Raw_Read_Write(part_settings))
	return false;
    }
...
bool TWPartition::Find_Wildcard_Block_Devices(const string & Device)
{
  ...
  while ((de = readdir(d)) != NULL)
    {
      if (de->d_type != DT_BLK || strlen(de->d_name) <= wildcard_index
	  || strncmp(de->d_name, Dev.c_str(), wildcard_index) != 0)
	continue;
	...
	continue;
      ...
      sprintf(buffer, "%s %s-%i auto defaults defaults", item.c_str(),
	      Mount_Point.c_str(), ++mount_point_index);
...
void TWPartition::Find_Actual_Block_Device(void)
{
  ...
  if (Wildcard_Block_Device && !Is_Adopted_Storage)
	...
	{
	  string Dev =
	    Primary_Block_Device.substr(0, Primary_Block_Device.find("*"));
      ...
      if (TWFunc::Path_Exists(Decrypted_Block_Device))
	{
	  ...
	  return;
	   ...
	   && TWFunc::Path_Exists(Primary_Block_Device +
				  PartitionManager.Get_Active_Slot_Suffix()))
    {
      Actual_Block_Device =
	Primary_Block_Device + PartitionManager.Get_Active_Slot_Suffix();
  ...
  if (!Alternate_Block_Device.empty()
      && TWFunc::Path_Exists(Alternate_Block_Device))
    {
  ...
  if (!Mount(true))
    {
      gui_msg(Msg
	      (msg::kError,
	       "recreate_folder_err=Unable to recreate {1} folder.")
	      (Media_Path));
    }
...
bool TWPartition::Flash_Image(PartitionSettings * part_settings)
{
  ...
  if (Backup_Method == BM_FILES)
	...
	{
	  LOGINFO
	    ("Size (%llu bytes) of image '%s' is larger than target device '%s' (%llu bytes)\n",
	     image_size, Backup_FileName.c_str(), Actual_Block_Device.c_str(),
	     Size);
  ...
  if (fd < 0)
    {
      gui_msg(Msg
	      (msg::kError,
	       "error_opening_strerr=Error opening: '{1}' ({2})") (Filename)
	      (strerror(errno)));
      return false;
    }
  ...
  if (Mount(false))
    {
      ...
      string file =
	"/sys/fs/" + Current_File_System + "/" + block +
...
int TWPartition::Decrypt_Adopted()
{
	  ...
	  if (TWFunc::Path_Exists(Adopted_Key_File))
	    {
	       ...
	       */
	      LOGINFO
		("Detected adopted storage, disabling USB mass storage mode\n");
  ...
  if (Is_Adopted_Storage)
      ...
      while ((n = read(fdkey, &buf[0], sizeof(buf))) > 0)
	{
      ...
      ret =
	cryptfs_setup_ext_volume(part_guid, Adopted_Block_Device.c_str(), key,
				 thekey.size(), crypto_blkdev);
      if (ret == 0)
	{
	  ...
	  if (!Mount_Storage_Retry(false))
	  ...
	  else
	    {
	      ...
	      Backup_Path = Mount_Point;
	      TWPartition *sdext =
		PartitionManager.Find_Partition_By_Path("/sd-ext");
	      if (sdext && sdext->Actual_Block_Device == Adopted_Block_Device)
		{
		  LOGINFO
		    ("Removing /sd-ext from partition list due to adopted storage\n");
